-- ============================================
-- ZIMCROWD TARGETED SCHEMA UPDATE
-- ============================================
-- Based on existing tables analysis
-- Only creates missing tables for new features
-- ============================================
...
-- ============================================
-- ✅ EXISTING: profiles, loans, investments, transactions, user_activity
-- ❌ MISSING: Payment system, Referral system, ZimScore & KYC tables
-- ============================================

-- ============================================
-- 1. PAYMENT SYSTEM TABLES (NEW)
-- ============================================

-- Payment transactions table (PayNow integration)
...
-- ============================================
-- 4. PERFORMANCE INDEXES (NEW TABLES ONLY)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_user_documents_verified ON user_documents(is_verified);
CREATE INDEX IF NOT EXISTS idx_zimscore_users_user_id ON zimscore_users(user_id);
CREATE INDEX IF NOT EXISTS idx_zimscore_users_score ON zimscore_users(current_score DESC);
CREATE INDEX IF NOT EXISTS idx_zimscore_users_kyc_status ON zimscore_users(kyc_status);
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(DISTINCT rc.id) FILTER (WHERE rc.conversion_type = 'signup') as successful_referrals,
        COALESCE(rcr.total_credits, 0) as total_credits_earned
    FROM referral_conversions rc
    LEFT JOIN referral_credits rcr ON rcr.user_id = p_user_id
    WHERE rc.referrer_id = p_user_id
    GROUP BY p_user_id, rcr.total_credits
...
SELECT 
    ...
    SUM(pt.amount) FILTER (WHERE pt.status = 'paid') as total_amount,
    AVG(pt.amount) FILTER (WHERE pt.status = 'paid') as avg_amount
FROM payment_transactions pt
GROUP BY DATE(pt.created_at), pt.currency, pt.payment_method
ORDER BY date DESC;
...
GROUP BY rl.user_id, p.first_name, p.last_name, p.email, rl.referral_code, rcr.total_credits, rcr.available_credits, rl.created_at;
...
LEFT JOIN profiles p ON p.id = zu.user_id
LEFT JOIN user_documents ud ON ud.user_id = zu.user_id
GROUP BY zu.user_id, p.first_name, p.last_name, p.email, zu.current_score, zu.kyc_status, zu.updated_at;

-- ============================================
-- 8. INITIAL DATA
-- ============================================

-- Insert default payment analytics for today
...
SELECT 
    'TARGETED SCHEMA UPDATE COMPLETE' as status,
...
-- ============================================
-- TARGETED SCHEMA UPDATE COMPLETE
-- ============================================
-- ✅ Preserves all existing tables and data
-- ✅ Adds only missing tables for new features  
-- ✅ Uses proper foreign key references to profiles table
