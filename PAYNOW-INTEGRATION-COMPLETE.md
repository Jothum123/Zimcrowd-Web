...
async function processLoanRepayment(userId, loanId, amount, currency = 'USD') {
    try {
        ...
        const response = await axios.post('https://zimcrowd-backend.vercel.app/api/payments/initiate/web', {
            amount,
...
async function pollPaymentStatus(pollUrl, reference) {
    ...
    const interval = setInterval(async () => {
        try {
            const response = await axios.get(`https://zimcrowd-backend.vercel.app/api/payments/status/${reference}`);
...
async function processEcoCashPayment(userId, amount, currency = 'USD') {
    try {
        ...
        const response = await axios.post('https://zimcrowd-backend.vercel.app/api/payments/initiate/mobile', {
            amount,
...
async function getUserPaymentHistory(userId, limit = 50) {
    try {
        const response = await axios.get(`https://zimcrowd-backend.vercel.app/api/payments/history/${userId}?limit=${limit}`);
